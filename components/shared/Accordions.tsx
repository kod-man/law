    ...
    <Accordion className="pt-3" type="single" collapsible>
      {data.map((item) => (
        <AccordionItem
          ...
          key={item.id}
        >
          <AccordionTrigger className="text-base text-background font-semibold py-2 px-4 border-2 border-background hover:no-underline rounded-md focus:bg-background focus:text-white">
            {item.question}
          </AccordionTrigger>
